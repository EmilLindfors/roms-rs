    ...
    for k in ElementIndex::iter(mesh.n_elements) {
        ...
        for face in 0..4 {
            ...
            let (u_ext, _use_bc) = if let Some(neighbor) = mesh.neighbor(k, face) {
                ...
                let u_neighbor = u.element(ElementIndex::new(neighbor.element));
...
mod tests {
    ...
    fn test_volume_term_only() {
        ...
        let elem = k(27); // 8x8 mesh, element (3,3)
    ...
    fn test_flux_types_give_same_for_continuous() {
        ...
        for ki in 0..mesh.n_elements {
            for i in 0..ops.n_nodes {
                let diff = (rhs_upwind.element(k(ki))[i] - rhs_lf.element(k(ki))[i]).abs();
                assert!(
                    diff < 1e-12,
    ...
    fn test_boundary_condition_inflow() {
        ...
        for ki in 0..mesh.n_elements {
            for i in 0..ops.n_nodes {
                if rhs.element(k(ki))[i].abs() > 1e-10 {
                    has_nonzero = true;
                }
            }
        }
