...
use crate::solver::rhs::advection_3d::{
    apply_horizontal_advection_3d, apply_vertical_advection_3d,
...
pub fn compute_rhs_3d(
    ...
    // Note: This computes the FULL pressure gradient (barotropic + baroclinic).
    ...
    apply_tracer_advection_3d(
        &mut rhs.temp, &state.temp, state, 
        config.mesh, config.ops, config.geom
    );
    apply_tracer_advection_3d(
        &mut rhs.salt, &state.salt, state, 
        config.mesh, config.ops, config.geom
    );
    
    // 7. Vertical Advection (All Fields)
    // Must be called after horizontal terms are accumulated, as it adds to them.
    apply_vertical_advection_3d(rhs, state, w_vel, config.sigma);
}
