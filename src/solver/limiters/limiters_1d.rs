...
fn cell_averages(q: &SWESolution, ops: &DGOperators1D) -> Vec<[f64; 2]> {
    ...
    for k in ElementIndex::iter(n_elements) {
        ...
        averages[k.as_usize()][0] = avg_h / weight_sum;
        averages[k.as_usize()][1] = avg_hu / weight_sum;
    }
...
pub fn tvb_limiter(q: &mut SWESolution, mesh: &Mesh1D, ops: &DGOperators1D, tvb: &TVBParameter) {
    ...
    for k in ElementIndex::iter(n_elements) {
        ...
        let avg_h = averages[k.as_usize()][0];
        let avg_hu = averages[k.as_usize()][1];
        ...
        let ki = k.as_usize();
        let (avg_h_left, avg_hu_left) = if mesh.neighbors[ki].0.is_some() {
            (averages[ki - 1][0], averages[ki - 1][1])
        } else {
            (avg_h, avg_hu) // Use own average at boundary
        };

        let (avg_h_right, avg_hu_right) = if mesh.neighbors[ki].1.is_some() {
            (averages[ki + 1][0], averages[ki + 1][1])
        } else {
            (avg_h, avg_hu) // Use own average at boundary
        };
...
pub fn characteristic_limiter(
    ...
    for k in ElementIndex::iter(n_elements) {
        let avg = SWEState::new(averages[k.as_usize()][0], averages[k.as_usize()][1]);
        ...
        let (avg_left, avg_right) = {
            let left = if mesh.neighbors[ki].0.is_some() {
                SWEState::new(averages[ki - 1][0], averages[ki - 1][1])
            } else {
                avg
            };
            let right = if mesh.neighbors[ki].1.is_some() {
                SWEState::new(averages[ki + 1][0], averages[ki + 1][1])
            } else {
                avg
            };
        ...
        if needs_limiting {
            ...
            for i in 0..n_nodes {
                ...
                let new_h = averages[k.as_usize()][0] + slope_h * xi;
                let new_hu = averages[k.as_usize()][1] + slope_hu * xi;
...
mod tests {
    ...
    fn test_tvb_limiter_smooth_preserves() {
        ...
        for ki in 0..10 {
            for i in 0..4 {
                let orig = q_orig.get(k(ki), i);
                let new = q.get(k(ki), i);
    ...
    fn test_tvb_limiter_preserves_average() {
        ...
        for ki in 0..10 {
            assert!(
                (avg_before[ki][0] - avg_after[ki][0]).abs() < 1e-12,
                "Cell average h not preserved"
            );
            assert!(
                (avg_before[ki][1] - avg_after[ki][1]).abs() < 1e-12,
