...
impl TracerBCContext2D {
    ...
    pub fn interior_normal_velocity(&self) -> f64 {
        let (u, v) = self.interior_swe.velocity_simple(self.h_min);
                ...
                if let Some(neighbor) = mesh.neighbor(k, face) {
                    ...
                    (0..n_face_nodes)
                        .map(|i| {
                            ...
                            (
                                tracers.get_conservative(neighbor.element, ni),
                                swe.get_state(neighbor.element, ni),
                            )
                        })
                        .collect()
                } else {
        ...
        if config.kappa_t > 0.0 || config.kappa_s > 0.0 {
            ...
            let concentrations: Vec<TracerState> = (0..n_nodes)
                .map(|i| {
                    ...
                    tracers.get_concentrations(k, i, h, h_min)
                })
                .collect();
