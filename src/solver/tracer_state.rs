...
impl TracerSolution2D {
    ...
    pub fn get_conservative(&self, k: usize, i: usize) -> ConservativeTracerState {
        let base = (k * self.n_nodes + i) * Self::N_VARS;
        ConservativeTracerState::from_array([self.data[base], self.data[base + 1]])
    }

    /// Set the conservative state at node i in element k.
    pub fn set_conservative(&mut self, k: usize, i: usize, state: ConservativeTracerState) {
        let base = (k * self.n_nodes + i) * Self::N_VARS;
    ...
    pub fn get_concentrations(&self, k: usize, i: usize, h: f64, h_min: f64) -> TracerState {
        self.get_conservative(k, i).to_concentrations(h, h_min)
    }

    /// Set from concentrations and water depth at node i in element k.
    pub fn set_from_concentrations(&mut self, k: usize, i: usize, h: f64, tracers: TracerState) {
        let cons = ConservativeTracerState::from_depth_and_tracers(h, tracers);
        self.set_conservative(k, i, cons);
    }

    /// Get hT (h * Temperature) at a node.
    pub fn get_h_t(&self, k: usize, i: usize) -> f64 {
        let base = (k * self.n_nodes + i) * Self::N_VARS;
        self.data[base]
    }

    /// Get hS (h * Salinity) at a node.
    pub fn get_h_s(&self, k: usize, i: usize) -> f64 {
        let base = (k * self.n_nodes + i) * Self::N_VARS;
        self.data[base + 1]
    }

    /// Set hT at a node.
    pub fn set_h_t(&mut self, k: usize, i: usize, value: f64) {
        let base = (k * self.n_nodes + i) * Self::N_VARS;
        self.data[base] = value;
    }

    /// Set hS at a node.
    pub fn set_h_s(&mut self, k: usize, i: usize, value: f64) {
        let base = (k * self.n_nodes + i) * Self::N_VARS;
    ...
    pub fn temperature_range(
        &self,
        swe_solution: &crate::solver::SWESolution2D,
        h_min: f64,
    ) -> (f64, f64) {
...
mod tests {
    ...
    fn test_tracer_solution_basic() {
        ...
        sol.set_from_concentrations(0, 0, h, tracers);

        let cons = sol.get_conservative(0, 0);
        assert!((cons.h_t - 60.0).abs() < TOL);
        assert!((cons.h_s - 165.0).abs() < TOL);

        let recovered = sol.get_concentrations(0, 0, h, 1e-6);
