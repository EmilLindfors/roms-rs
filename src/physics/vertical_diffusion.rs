...
use crate::physics::vertical_mixing::{VerticalMixing, Column, Forcing};
...
pub fn apply_vertical_diffusion<M: VerticalMixing + ?Sized>(
    ...
    for k in 0..state.n_elements {
        for i in 0..state.n_nodes {
            ...
            let h = 100.0; // TODO: Get actual depth
            ...
            // Store diagnostics
            state.eddy_viscosity_column_mut(elem_idx, i).copy_from_slice(&av[0..n_levels]);
            state.eddy_diffusivity_column_mut(elem_idx, i).copy_from_slice(&kt[0..n_levels]);
            ...
            solve_diffusion_column(
                ...
                &mut a, &mut b, &mut c, &mut d, &mut x, &mut c_prime, &mut d_prime
            );
...
fn solve_diffusion_column(
    ...
    flux_top: f64,
    flux_bot: f64,
    a: &mut [f64], b: &mut [f64], c: &mut [f64], d: &mut [f64],
    x: &mut [f64], c_prime: &mut [f64], d_prime: &mut [f64],
) {
    let n = phi.len();
...
mod tests {
    ...
    fn test_diffusion_constant() {
        ...
        solve_diffusion_column(
            &mut phi, &nu, &dz, dt, flux_top, flux_bot,
            &mut a, &mut b, &mut c, &mut d, &mut x, &mut c_prime, &mut d_prime
        );
    ...
    fn test_diffusion_linear_flux() {
        ...
        for i in 0..n {
            assert!((phi[i] - phi_orig[i]).abs() < 1e-10, "Changed at {}: {} -> {}", i, phi_orig[i], phi[i]);
        }
    }
}
